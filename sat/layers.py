...
class Attention(gnn.MessagePassing):
    ...
    def __init__(self, embed_dim, num_heads=8, dropout=0., bias=False,
        ...
        self.neighboor_fc = nn.Linear(embed_dim, embed_dim)
    ...
    def forward(self,
        ...
        x_1hop_neighboor = self.neighboor_fc(scatter(x[edge_index[0]], edge_index[1], dim=0, dim_size=x.size(0), reduce='sum'))
        ...
        if complete_edge_index is not None:
            out = self.propagate(complete_edge_index, v=v, qk=qk, edge_attr=None, size=None,
                                 return_attn=return_attn, complete_edge_attr=complete_edge_attr)
            if return_attn:
                attn = self._attn
                self._attn = None
            ...
            out = rearrange(out, 'n h d -> n (h d)') + x_1hop_neighboor
        else:
            out, attn = self.self_attn(qk, v, ptr, return_attn=return_attn)
        return self.out_proj(out), attn

    def message(self, v_j, qk_j, qk_i, edge_attr, index, ptr, size_i, return_attn, complete_edge_attr=None):
        ...
        if complete_edge_attr is not None:
            complete_edge_attr = rearrange(complete_edge_attr, 'n (h d) -> n h d', h=self.num_heads)
            attn = (qk_i * qk_j + complete_edge_attr).sum(-1) * self.scale
        else:
...
class TransformerEncoderLayer(nn.TransformerEncoderLayer):
    ...
    def forward(self, x, edge_index, complete_edge_index, complete_edge_attr=None,
        ...
        x2 = self.linear2(F.glu(self.linear1(x), dim=-1))
