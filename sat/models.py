...
class GraphTransformerEncoder(nn.TransformerEncoder):
    def forward(self, x, edge_index, complete_edge_index, complete_edge_attr=None,
        ...
        for mod in self.layers:
            output = mod(output, edge_index, complete_edge_index, complete_edge_attr,
...
class positional_encoder(nn.Module):
    def __init__(self, hid_dim, max_len=3000):
        super(positional_encoder, self).__init__()
        # Compute the positional encodings once in log space.
        pe = torch.zeros(max_len, hid_dim)
        position = torch.arange(0, max_len).unsqueeze(1)
        div_term = torch.exp((torch.arange(0, hid_dim, 2)).type(torch.FloatTensor) * (-(math.log(10000.0) / hid_dim)))
    ...
    def forward(self, x):
        x = x + self.pe[0:x.size(1), :x.size(2)].detach().requires_grad_(False)
        return x
...
class GraphTransformer(nn.Module):
    def __init__(self, in_size, d_model, num_heads=8,
                 ...
                 in_embed=False, edge_embed=False, use_global_pool=True, use_seq_pos=False,
                 global_pool='mean', complete_att_edge=16, **kwargs):
        super().__init__()
        ...
        self.complete_edge_attr_fc = nn.Linear(complete_att_edge, d_model)
        self.positional_encoder = positional_encoder(d_model)
    ...
    def forward(self, data,return_attn=False):
        ...
        complete_edge_attr = data.complete_edge_attr if hasattr(data, 'complete_edge_attr') else None

        # 带有位置的边信息
        if complete_edge_attr is not None:
            complete_edge_attr = self.complete_edge_attr_fc(complete_edge_attr)
            # complete_edge_attr = self.complete_edge_attr_embedding(complete_edge_attr)
        ...
        # #添加位置信息
        # if self.use_seq_pos:
        #     output, protein_mask = to_dense_batch(output, data.batch)
        ...
        if self.global_pool == 'cls' and self.use_global_pool:
            ...
            if complete_edge_index is not None:
                # print('---', torch.arange(data.num_nodes))
                # print('+++', data.batch, 'num_nodes = ', data.num_nodes)
        ...
        output = self.encoder(
            ...
            edge_index, 
            complete_edge_index,
            complete_edge_attr=complete_edge_attr,
            edge_attr=edge_attr, 
            degree=degree,
            subgraph_node_index=subgraph_node_index,
            subgraph_edge_index=subgraph_edge_index,
        ...
        if self.use_global_pool:
            if self.global_pool == 'cls':
                ...
                output_g = self.pooling(output, data.batch)

        # return self.classifier(output)
        return output, output_g