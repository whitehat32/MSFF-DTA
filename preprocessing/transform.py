...
from .transform_utils.subgraph_extractors import metis_subgraph, random_subgraph
from .data_utils.pe import add_positional_encoding
import re
...
class RandomPartitionTransform(object):
    ...
    def __call__(self, data):
        ...
        # print('node_masks = ', node_masks.shape, 'edge_masks = ', edge_masks.shape)
        subgraphs_nodes, subgraphs_edges = to_sparse(node_masks, edge_masks)
        # print('subgraph_nodes = ', subgraphs_nodes.shape, 'subgraph_edges = ', subgraphs_edges.shape)
        ...
        # print('combined_subgraphs = ', combined_subgraphs.shape)
        coarsen_adj = cal_coarsen_adj(node_masks)
        # print('coarsen_adj = ', coarsen_adj.shape)
        data.coarsen_adj = random_walk(coarsen_adj)
        # print('data.coarsen_adj = ', data.coarsen_adj.shape)
        subgraphs_batch = subgraphs_nodes[0]
        # print('subgraphs_batch = ', subgraphs_batch.shape)
...
class MetisPartitionTransform(object):
    def __init__(self, n_patches, drop_rate=0.0, num_hops=1, is_directed=False):
        super().__init__()
        self.n_patches = n_patches
    ...
    def __call__(self, data):
        ...
        # data.coarsen_adj = random_walk(coarsen_adj)
        Dinv = coarsen_adj.sum(dim=-1).clamp(min=1).pow(-1).unsqueeze(-1)  # D^-1
        data.coarsen_adj = coarsen_adj * Dinv
        # data.coarsen_adj = coarsen_adj
        subgraphs_batch = subgraphs_nodes[0]
        mask = torch.zeros(self.n_patches).bool()
        mask[subgraphs_batch] = True
        data.subgraphs_batch = subgraphs_batch
